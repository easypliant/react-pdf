...
describe("stylesheet transform", () => {
  ...
  describe("transform transform", () => {
    ...
    test("should transform multiple value scale operation with negative and floats", () => {
      ...
      expect(style.transform).toEqual([{ operation: "scale", value: [0.1, -10] }]);
    });
    ...
    test("should transform translate operation", () => {
      ...
      expect(style.transform).toEqual([{ operation: "translate", value: [10, 20] }]);
    });
    ...
    test("should transform rad rotate operation", () => {
      ...
      expect(style.transform).toEqual([{ operation: "rotate", value: [180.0004209182994] }]);
    });
    ...
    test("should transform float skew operation", () => {
      ...
      expect(style.transform).toEqual([{ operation: "skew", value: [30.5, 40.5] }]);
    });
    ...
    test("should transform mixed units skew operation", () => {
      ...
      expect(style.transform).toEqual([{ operation: "skew", value: [-180.0004209182994, 10] }]);
    });
    ...
    test("should transform matrix operation", () => {
      ...
      expect(style.transform).toEqual([{ operation: "matrix", value: [-1, -0.1, 0, 0.1, 1, 10] }]);
    });

    test("should transform combined operations", () => {
      const style = transformStyles({
        transform: "translate(10px, 20px) scale(0.1, -10) matrix(-1, -0.1, 0, 0.1, 1, 10)",
      });
