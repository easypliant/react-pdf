...
describe("layout resolvePageSizes", () => {
  ...
  test("Should accept size string in landscape mode", () => {
    const root = {
      type: "DOCUMENT",
      children: [{ type: "PAGE", props: { size: "A2", orientation: "landscape" } }],
    };
    const result = resolvePageSizes(root);
  ...
  test("Should accept size object", () => {
    const root = {
      type: "DOCUMENT",
      children: [{ type: "PAGE", props: { size: { width: 100, height: 200 } } }],
    };
    const result = resolvePageSizes(root);
  ...
  test("Should accept size number in landscape mode", () => {
    const root = {
      type: "DOCUMENT",
      children: [{ type: "PAGE", props: { size: 100, orientation: "landscape" } }],
    };
    const result = resolvePageSizes(root);
